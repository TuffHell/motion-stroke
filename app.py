...
if st.sidebar.button("▶️ Initialize AI Biomechanical Scan"):
    with st.spinner(f"Rendering {num_frames} High-Fidelity Kinematic Frames..."):
        ...
        # ---------------------------------------------------------
        # 7. UI TABS: FIVE DEDICATED SECTIONS
        # ---------------------------------------------------------
        ...
        # --- TAB 2: UPPER BODY (WRISTS) ---
        with tab2:
            st.markdown("### 🦾 Upper Extremity Biomechanics")
            st.subheader("CNN Tremor Detection: Wrist Acceleration (m/s²)")
            fig_acc_arm = go.Figure()
            fig_acc_arm.add_trace(go.Scatter(y=left_wrist_y_acc, mode='lines', name='Left Wrist Accel', line=dict(color='red' if is_stroke and "Left" in patient_type else 'cyan', width=2)))
            fig_acc_arm.add_trace(go.Scatter(y=right_wrist_y_acc, mode='lines', name='Right Wrist Accel', line=dict(color='orange' if is_stroke and "Right" in patient_type else 'lightgreen', width=2)))
            fig_acc_arm.update_layout(height=350, margin=dict(t=10, b=10))
            st.plotly_chart(fig_acc_arm, use_container_width=True)

            col_x1, col_x2 = st.columns(2)
            with col_x1:
                st.subheader("CNN Spasticity Filter (Amplitude)")
            ...
            with col_x2:
                st.subheader("LSTM Bradykinesia Filter (Rhythm)")
                fig_lstm_arm = go.Figure()
                fig_lstm_arm.add_trace(go.Scatter(y=features_24d[:, 18], mode='lines', name='Left Arm Energy', line=dict(color='red' if is_stroke and "Left" in patient_type else 'cyan', width=3)))
                fig_lstm_arm.add_trace(go.Scatter(y=features_24d[:, 19], mode='lines', name='Right Arm Energy', line=dict(color='orange' if is_stroke and "Right" in patient_type else 'lightgreen', width=3, dash='dot')))
                fig_lstm_arm.update_layout(height=350, margin=dict(t=10, b=10))
                st.plotly_chart(fig_lstm_arm, use_container_width=True)

        # --- TAB 3: LOWER BODY (ANKLES) ---
        with tab3:
            st.markdown("### 🦵 Lower Extremity Biomechanics")
            st.subheader("CNN Gait-Drag Detection: Ankle Acceleration (m/s²)")
            st.info("Notice how severely the acceleration drops on the affected side if the patient is dragging their foot.")
            fig_acc_leg = go.Figure()
            fig_acc_leg.add_trace(go.Scatter(y=left_ankle_y_acc, mode='lines', name='Left Ankle Accel', line=dict(color='darkred' if is_stroke and "Left" in patient_type else 'blue', width=2)))
            fig_acc_leg.add_trace(go.Scatter(y=right_ankle_y_acc, mode='lines', name='Right Ankle Accel', line=dict(color='darkorange' if is_stroke and "Right" in patient_type else 'green', width=2)))
            fig_acc_leg.update_layout(height=350, margin=dict(t=10, b=10))
            st.plotly_chart(fig_acc_leg, use_container_width=True)
            
            st.subheader("Lower Body Kinetic Energy Output")
            fig_lstm_leg = go.Figure()
            fig_lstm_leg.add_trace(go.Scatter(y=features_24d[:, 20], mode='lines', name='Left Leg Energy', line=dict(color='darkred' if is_stroke and "Left" in patient_type else 'blue', width=3)))
            fig_lstm_leg.add_trace(go.Scatter(y=features_24d[:, 21], mode='lines', name='Right Leg Energy', line=dict(color='darkorange' if is_stroke and "Right" in patient_type else 'green', width=3, dash='dot')))
            fig_lstm_leg.update_layout(height=350, margin=dict(t=10, b=10))
            st.plotly_chart(fig_lstm_leg, use_container_width=True)
